...
export default function VocabularyClient({
  ...
  if (mode === "flashcard" && filteredVocabs.length > 0) {
    ...
    return (
      <div>
        <PageHeader
          title="Flashcard"
          subtitle={`${currentIndex + 1} / ${shuffledVocabs.length}`}
          actions={
            <button
    ...
    return (
      <div>
        <PageHeader
          ...
          subtitle={`${progress.length} từ đã lưu`}
          actions={
        ...
        {filteredVocabs.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredVocabs.map((vocab) => (
              <Card key={vocab.id} hover className="group">
  ...
  return (
    <div>
      <PageHeader
        title="Từ vựng HSK"
        subtitle="Học từ vựng theo cấp độ HSK"
        actions={
          progress.length > 0 && (
            <button
              onClick={() => setMode("mywords")}
