...
export default function AboutWhyChooseUs() {
  const reasons = [
    ...
    },
    {
      ...
      iconBg: "bg-linear-to-br from-yellow-500 to-orange-600",
      badge: "Chuyên nghiệp",
      accentIcon: Zap,
    },
    {
  ...
  return (
    <div className="relative mb-24">
      ...
      <div className="absolute inset-0 bg-linear-to-r from-red-100/50 via-yellow-100/50 to-orange-100/50 dark:from-red-900/10 dark:via-yellow-900/10 dark:to-orange-900/10 rounded-3xl blur-3xl -z-10" />
      ...
      <div className="rounded-3xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border border-border-light dark:border-border-dark shadow-2xl overflow-hidden">
        {/* Header Section */}
        <div className="relative bg-linear-to-r from-red-600 via-orange-500 to-yellow-500 px-8 py-12 md:px-12 md:py-20 text-center text-white overflow-hidden">
          ...
          {reasons.map((reason, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`absolute -inset-1 bg-linear-to-r ${reason.color} opacity-0 group-hover:opacity-20 blur-xl transition-opacity rounded-2xl`}></div>

                <div className={`relative bg-linear-to-br ${reason.bgColor} border-2 border-gray-200 dark:border-gray-700 group-hover:border-transparent p-8 rounded-2xl shadow-xl transition-all duration-300 hover:-translate-y-3 hover:shadow-2xl overflow-hidden`}>
                  {/* Background decoration */}
                  <div className="absolute top-0 right-0 w-32 h-32 opacity-5 group-hover:opacity-10 transition-opacity">
                    <div className={`w-full h-full bg-linear-to-br ${reason.color} rounded-full blur-2xl`}></div>
                  </div>
                  ...
                  <div className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-md">
                    <AccentIcon className={`w-3 h-3 bg-linear-to-r ${reason.color} bg-clip-text text-transparent`} />
                  ...
                  <div className={`relative inline-flex w-16 h-16 rounded-2xl ${reason.iconBg} shadow-lg mb-6 items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-transform`}>
                    ...
                    {/* Glow effect */}
                    <div className={`absolute inset-0 rounded-2xl bg-linear-to-r ${reason.color} blur-md opacity-50 group-hover:opacity-75 transition-opacity`}></div>
                  </div>
                  ...
                  {/* Hover accent line */}
                  <div className={`absolute bottom-0 left-0 right-0 h-1 bg-linear-to-r ${reason.color} transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left`}></div>
                </div>
              </div>
            );
          })}
